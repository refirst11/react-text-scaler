...
export const TextScaler = ({
  ...
  useLayoutEffect(() => {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      element.style.setProperty('font-size', scaleFontSize < 10 ? '10px' : scaleFontSize + 'px');
    }

    if (text) {
      const initialFontSize = fontSizes[fontSizes.length - 1];
      const scaleFontSize = initialFontSize * scale;
      text.style.setProperty('font-size', scaleFontSize < 10 ? '10px' : scaleFontSize + 'px');
    }
  ...
  }, [handleMouse, handleMouseDown, handleMouseUp, handleTouchMove, handleTouchStart, handleWheel]);
  ...
  const classes2 = styles.center + ' ' + (visible ? styles.visible : styles.hidden);
  ...
  return (
    <div
      ...
      className={classes1}
      style={{
        border: center && visible ? 'none' : border,
        boxShadow: center && visible ? 'none' : boxShadow,
        backgroundColor: center && visible ? 'inherit' : backgroundColor,
      }}
